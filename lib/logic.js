 ...
 *///是否需要创建被封禁的注册表
 function sealAccount(company2){

    //主要涉及的操作是检查资源是否存在
    //获取这个资源（不使用解析）
    //更新这个资源
    //ps:可以通过getCurrentParticipant获取调用参与者的详细信息4
  ...
  /* 
    //检测是用户是否存在
     return CompanyRegistry
        .then(function(function(participantRegistry){
            return participantRegistry.exists(companyBeSealed.email);
        })
        .then(function(exist){
            console.log('Company exist',exist);
        })
        .catch(function(error){
        });
  */
        //硬编码
    if(!CompanyRegistry.exsits(company2.email)){
        throw new Error('Sealed Account'+company2.email+'does not exist!');
    }

    targertCompany = await CompanyRegistry.get(company2.email);
    targertCompany.state='N';//Y为激活，N为禁用
    await CompanyRegistry.update(targertCompany);
}
 ...
 */
function unsealedAccount(company2){
    var NS='token';
    const CompanyRegistry = await getParticipantRegistry(NS+'.Company');

    if(!CompanyRegistry.exists(company2.email))
    {
        throw new Error('Unsealed Account'+company2.email+'does not exist!')
    }

    targertCompany = await CompanyRegistry.get(company2.email);
    targertCompany.state='Y';//Y为激活，N为禁用
    await CompanyRegistry.update(targertCompany);
...
/**
 * 
 * @param{token.TransactionRollback} transactionRollback
 */
funtion transactionRollback(trasactionBack2){
    var NS='token';
    var factory = getFactory();
    var userTransactionRollback = factory.newResource(NS,'TransactionBack',transactionBack2.transanctionBackId);
        userTransactionRollback.fromEmail = trasactionBack2.fromEmail;
        userTransactionRollback.toEmail = trasactionBack2.toEmail;
        userTransactionRollback.transactionBackNum = trasactionBack2.transactionBackNum;

    //回退操作上链
    const TransactionRollback =getTransactionRegistry(...)//待交易组写完补上
    await TransactionRollback.update(userTransactionRollback);
    
    //支付方交易数据修改并上链
    //1、通过email找到支付方的交易链
    //2、新生成一笔交易，这笔交易的使得支付方的余额增加相应数额
    //3、交易上链
    const transactionOfPayer = await getTransactionRegistry(...);//待交易组写完补上
    targertTransactionOfPayer = transactionOfPayer.get(userTransactionRollback.fromEmail);
  

   //收受方交易数据修改并上链
    const transcationOfRecipient = getTransactionRegistry(...);//待交易组写完补上
    targertTransactionOfRecipient = transcationOfRecipient.get(userTransactionRollback.fromEmail);
    ...
 ...
 * @param{token.UserFine} userFine
 */
async function userPenalty(penalty2){
    var NS = 'token';
    var factory = getFactory();
    var userTokenPenalty = factory.newResource(NS,'Penalty',penalty2.PenaltyID);//model里id是大写的
    userTokenPenalty.penaltyNum = penalty2.penaltyNum;
    userTokenPenalty.toUsersEmail = penalty2.toUsersEmail;
 ...
 * @param form； to； serviceID； complaintType；
 * @transaction
 * 需要初始化字段complaintId  fromEmail  toEmail complaintServiceId complaintType approveInfo 
 * approveState punishmentState
 * complaintState : 1 for 正常进行 ; -1 for 已经结束  其他  
*/
function initCompaint(from,to,serviceId,complaintType)
{
    var factory = getFactory();
    var NS = 'token';

    var Complaint = factory.newResource(NS, 'Compaint',Complaint.complaintId);
    // Complaint.complaintId=generateId();
    Complaint.complaintId=complaintId;
    Complaint.fromEmail=from;
    Complaint.toEmail=to;
    Complaint.complaintServiceId=serviceId;
    Complaint.complaintType=complaintType;
    // Complaint.approveInfo={};
    Complaint.approveState=1;//
    Complaint.punishmentState=0;


    return getAssetRegistry(NS + '.Compaint')
        .then(function (CompaintRegistry) {
            // add the growers
            return CompaintRegistry.addAll([Compaint]);
        });
}
...
{
    var NS = 'token';
    var appInfo = {adminId:approveResult};
    var penalty = {adminId:PenaltyId}
    var punishments = processComplaint.punishmentType.slice(0)
    //读取投诉流程状态
    if(processComplaint.ComplaintState==-1){
        throw new Error('The complaint has already over!.');
    }
    if(processComplaint.ComplaintState==0){
        throw new Error('The complaint is not existed!.');
    }

    //投诉实体内添加管理员审核的信息 adminId approveResult
    const ComplaintRegistry = await getAssetRegistry(NS + '.Compaint');
    const complaint = await ComplaintRegistry.get(processComplaint.complaintId);
    complaint.approveInfo.push(appInfo);
    // 改变审核流程的状态，何时结束
    if(approveResult==0) complaint.approveState=-1;//当前管理员审核不通过
    else if(approveInfo.length==need)//所有管理员审核结束
    {
        complaint.approveState=-1;
    }
    var i=0
    for(;i<punishments.length;i++)
    {
         //调用处罚函数处罚
        if(processPunishment(processComplaint.toEmail,punishments[i])){
            continue;
        }else{
            throw new Error('punish failed');
        }
         
    }
    // 改变处罚状态
    if(i==punishments.length-1){
        complaint.PunishmentState=1;//处罚完成
    }
    //最后返回更新的Complaint
    return getParticipantRegistry(NS + '.Complaint')
